import { Image, ImagePlus, Loader2, Plus, UploadCloud, X, ChevronDown, ChevronUp } from "lucide-react";
        ...
        {loading ? (
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {photos.map((photo) => {
              ...
              const descriptionLines = photo.description?.split('\n') || [];
              const shouldShowMoreButton = photo.description && photo.description.length > 100;
              ...
              return (
                <article key={photo.id} className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
                  ...
                  <div className="p-5">
                    ...
                    <p className={`mt-2 text-sm leading-6 text-slate-600 ${isExpanded ? '' : 'line-clamp-2'}`}>
                      {photo.description}
                    </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          <form onSubmit={handleCreate} className="w-full max-w-lg rounded-md bg-white p-5 shadow-xl">
            ...
            <div className="mt-4 grid gap-3">
              <input required value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Photo title" className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
              <input value={form.eventId} onChange={(event) => setForm((current) => ({ ...current, eventId: event.target.value }))} placeholder="Related event ID or category" className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
              <textarea rows={4} value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Caption or description" className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
